...
@objcMembers public class RoutingImpl: NSObject {

  public typealias CalculateRouteCompletion = (NSDictionary) -> Void
  public typealias CancelCompletion = (Bool) -> Void
  ...
  public func calculateRoute(
    waypoints: NSArray,
    routeOption: String,
    completion: @escaping CalculateRouteCompletion
  ) {
    taskHandle = routingEngine.calculateRoute(
      with: toCoordinatesList(raw: waypoints).map { Waypoint(coordinates: $0) },
  ...
  public func cancel(completion: @escaping CancelCompletion) {
