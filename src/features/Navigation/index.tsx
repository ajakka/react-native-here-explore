...
import {
  COMMAND_PREFETCH_USER_LOCATION,
 ...
 */
export const Navigation = forwardRef<NavigationHandle, NavigationProps>(
  ({ style, isSimulated = false, ...otherProps }, ref) => {
    ...
    useImperativeHandle(ref, () => ({
      ...
      prefetchUserLocation: () => {
        const viewTag = findNodeHandle(nativeViewRef.current);
        if (viewTag) {
          UIManager.dispatchViewManagerCommand(viewTag, COMMAND_PREFETCH_USER_LOCATION, []);
        } else {
          console.error('Could not find native view handle for prefetching user location');
        }
      },
    }));

    return (
      <RCTNavigationView ref={nativeViewRef} isSimulated={isSimulated} style={[{ flex: 1 }, style]} {...otherProps} />
    );
  }
);
